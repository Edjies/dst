...
package com.haofeng.apps.dst.utils.clusterutil.quadtree;
...
/**
 * A quad tree which tracks items with a Point geometry. See
 * http://en.wikipedia.org/wiki/Quadtree for details on the data structure. This
...
public class PointQuadTree<T extends PointQuadTree.Item> {
	...
	private PointQuadTree(double minX, double maxX, double minY, double maxY,
			int depth) {
	...
	private void split() {
		...
		mChildren.add(new PointQuadTree<T>(mBounds.minX, mBounds.midX,
				mBounds.minY, mBounds.midY, mDepth + 1));
		mChildren.add(new PointQuadTree<T>(mBounds.midX, mBounds.maxX,
				mBounds.minY, mBounds.midY, mDepth + 1));
		mChildren.add(new PointQuadTree<T>(mBounds.minX, mBounds.midX,
				mBounds.midY, mBounds.maxY, mDepth + 1));
		mChildren.add(new PointQuadTree<T>(mBounds.midX, mBounds.maxX,
				mBounds.midY, mBounds.maxY, mDepth + 1));
