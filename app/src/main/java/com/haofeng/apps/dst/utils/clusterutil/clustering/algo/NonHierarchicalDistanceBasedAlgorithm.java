...
package com.haofeng.apps.dst.utils.clusterutil.clustering.algo;
...
import com.haofeng.apps.dst.utils.clusterutil.projection.Bounds;
...
import com.haofeng.apps.dst.utils.clusterutil.quadtree.PointQuadTree;
...
public class NonHierarchicalDistanceBasedAlgorithm<T extends ClusterItem>
	...
	public Set<? extends Cluster<T>> getClusters(double zoom) {
		...
		final double zoomSpecificSpan = MAX_DISTANCE_AT_ZOOM
		...
		synchronized (mQuadTree) {
			for (QuadItem<T> candidate : mItems) {
				...
				Bounds searchBounds = createBoundsFromSpan(
				...
				for (QuadItem<T> clusterItem : clusterItems) {
					Double existingDistance = distanceToCluster
							.get(clusterItem);
					double distance = distanceSquared(clusterItem.getPoint(),
							candidate.getPoint());
