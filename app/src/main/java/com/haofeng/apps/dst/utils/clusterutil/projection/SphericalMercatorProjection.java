 ...
 */
package com.haofeng.apps.dst.utils.clusterutil.projection;
...
public class SphericalMercatorProjection {
	...
	public Point toPoint(final LatLng latLng) {
		...
		final double y = 0.5 * Math.log((1 + siny) / (1 - siny))
				/ -(2 * Math.PI) + .5;
	...
	public LatLng toLatLng(Point point) {
		...
		final double lat = 90 - Math.toDegrees(Math.atan(Math.exp(-y * 2
				* Math.PI)) * 2);
